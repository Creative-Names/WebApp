...
@customer_auth_bp.route('/register', subdomain='explore', methods=["GET", "POST"], strict_slashes=False)
def register_customer():
    ...
    if request.method == 'POST':
        ...
        if not emailIsUnique is None:
            flash(Markup(
                f"An account already exists with this email. <a href=\"{HTTP_STATUS}explore.{SERVER_NAME}/login\" class=\"alert-link\">Login Here.</a>"))
            return render_template('explorer_register.html')

        if not emailIsShopkeeper is None:
            print(email)
            flash(Markup(
                f"This email is already registered as a Business. <a href=\"{HTTP_STATUS}employer.{SERVER_NAME}/login\" class=\"alert-link\">Login Here.</a> Or try to register with a different email-id."))
...
@customer_auth_bp.route('/register/verify_otp', subdomain='explore', methods=["GET", "POST"], strict_slashes=False)
def verify_customer_reg_otp():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return redirect(f'{HTTP_STATUS}explore.{SERVER_NAME}/register')
        ...
        if otps[mail][1] != user_otp:
            flash(Markup(
                f"Wrong OTP. Try Again. Or <a href=\"{HTTP_STATUS}explore.{SERVER_NAME}/register\" class=\"alert-link\">Click here</a> to start over.</a>"))
        ...
        if otps[mail][1] == user_otp:
            ...
            try:
                ...
                db.session.commit()
                flash(Markup(
                    f"Accuount created successfully! <a href=\"{HTTP_STATUS}explore.{SERVER_NAME}/login\" class=\"alert-link\">Login Here.</a>"))
...
@customer_auth_bp.route('/login', subdomain='explore', methods=["GET", "POST"], strict_slashes=False)
def login_customer():
    if current_user.is_authenticated:
        return redirect(f'{HTTP_STATUS}explore.{SERVER_NAME}')
    if request.method == "POST":
        ...
        if user is None:
            flash(Markup(
                f"This Email is not registered yet! <a href=\"{HTTP_STATUS}explore.{SERVER_NAME}/register\" class=\"alert-link\">Signup Here</a>"))
...
@customer_auth_bp.route('/forgotPwd', subdomain='explore', methods=["GET", "POST"], strict_slashes=False)
...
@customer_auth_bp.route('/forgotPwd/verify_otp', subdomain='explore', methods=["GET", "POST"], strict_slashes=False)
def verify_customer_rec_otp():
    ...
    if request.method == 'POST':
        ...
        if rec_mail not in list(otps.keys()):
            ...
            return redirect(f'{HTTP_STATUS}explore.{SERVER_NAME}/forgotPwd')


        if otps[rec_mail] != user_rec_otp:
            flash(Markup(
                f"Wrong OTP. Try Again. Or <a href=\"{HTTP_STATUS}explore.{SERVER_NAME}/forgotPWD\" class=\"alert-link\">Click here</a> to start over.</a>"))
...
@customer_auth_bp.route('/logout', subdomain='explore', strict_slashes=False)
@login_required
def customer_logout():
    logout_user()
    return redirect(f"{HTTP_STATUS}explore.{SERVER_NAME}")
